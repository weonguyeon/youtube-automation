...
class TextShortsRenderer(BaseRenderer):
    """텍스트 기반 숏폼 인포그래픽 렌더러"""
    ...
    def render(self, script: VideoScript, output_dir: Path) -> VisualResult:
        ...
        for scene in script.scenes:
            img_path = output_dir / f"scene_{scene.scene_id:03d}.png"
            self._generate_scene_image(scene, theme, w, h, img_path)
            result.asset_paths.append(img_path)

        logger.info("텍스트 숏폼 에셋 %d개 생성 완료", len(result.asset_paths))
        return result
    ...
    def _generate_flux_image(self, prompt: str, w: int, h: int, output_path: Path):
        ...
        for _ in range(30):
            ...
            if data["status"] == "Ready":
                ...
                logger.info("Flux 이미지 생성 완료: %s", output_path.name)
                return
            elif data["status"] == "Error":
                raise RuntimeError(f"Flux API 에러: {data.get('error', 'unknown')}")

        raise TimeoutError("Flux API 응답 시간 초과 (60초)")

    def _generate_infographic_card(
        self, scene: Scene, theme: ColorTheme, w: int, h: int, output_path: Path
    ):
        """브랜드 컬러 기반 고품질 인포그래픽 카드 생성"""
        ...
        if scene_type == SceneType.HOOK:
            self._draw_hook_card(draw, subtitle, theme, cx, w, h, font_title, font_small, text_color)
        elif scene_type == SceneType.CONTENT:
            self._draw_content_card(draw, scene, subtitle, theme, cx, w, h, font_title, font_body, font_small, text_color)
        elif scene_type == SceneType.WRAPUP:
            self._draw_wrapup_card(draw, subtitle, theme, cx, w, h, font_title, font_body, font_small, text_color)
        elif scene_type == SceneType.CTA:
            self._draw_cta_card(draw, subtitle, theme, cx, w, h, font_title, text_color)
    ...
    def _draw_hook_card(self, draw, subtitle, theme, cx, w, h, font_title, font_small, text_color):
        """Hook: 대형 텍스트 + 원형 강조"""
        # 상단 레이블 뱃지
        draw.rounded_rectangle([cx - 100, 200, cx + 100, 240], radius=20, fill=theme.accent)
        ...
        r2 = r - 20
        draw.ellipse([cx - r2, cy - r2, cx + r2, cy + r2], outline=theme.primary, width=2)
        _draw_centered_text(draw, subtitle, font_title, text_color, cx, cy, w - 200)

        # 하단 장식 바
        bar_y = h * 3 // 4
        draw.rounded_rectangle([80, bar_y, w - 80, bar_y + 8], radius=4, fill=theme.accent)

    def _draw_content_card(self, draw, scene, subtitle, theme, cx, w, h, font_title, font_body, font_small, text_color):
        ...
        # 카드 내부 자막 텍스트
        _draw_centered_text(draw, subtitle, font_title, text_color, cx, card_top + 100, w - margin * 2 - 60)
    ...
    def _draw_wrapup_card(self, draw, subtitle, theme, cx, w, h, font_title, font_body, font_small, text_color):
        """Wrapup: 요약 카드 + CTA"""
        ...
        _draw_centered_text(draw, subtitle, font_title, text_color, cx, summary_top + 130, w - 200)
    ...
    def _draw_cta_card(self, draw, subtitle, theme, cx, w, h, font_title, text_color):
        """CTA: 풀스크린 강조"""
        ...
        _draw_centered_text(draw, subtitle, font_title, theme.background, cx, cy, r * 2 - 40)
...
def _draw_centered_text(
    draw: ImageDraw.Draw, text: str, font, color: str,
    cx: int, cy: int, max_width: int,
):
    """텍스트를 중앙 정렬로 그리기 (자동 줄바꿈)"""
    if not text:
        return
    words = list(text)  # 한글은 글자 단위 분리
    lines = []
    current = ""
    for char in words:
        test = current + char
    ...
    line_height = draw.textbbox((0, 0), "가Ag", font=font)[3] + 8
